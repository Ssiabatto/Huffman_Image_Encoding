...
from huffman import huffman_encode, huffman_decode, build_codes, calculate_efficiency, HuffmanNode, build_huffman_tree, tuple_huffman_decode
...
from PyQt5.QtWidgets import QApplication, QLabel, QPushButton, QVBoxLayout, QWidget, QHBoxLayout, QFileDialog, QGraphicsView, QGraphicsScene, QGraphicsEllipseItem, QGraphicsLineItem, QStackedWidget, QGraphicsPixmapItem, QGraphicsRectItem, QGridLayout
...
class ImageWindow(QWidget):
    def __init__(self, title, pixmap, info, stacked_widget):
        ...
        back_button.setStyleSheet("font-size: 18px; padding: 10px; background-color: #f0f0f0;")
    ...
    def scale_image_to_fit_view(self, pixmap_item):
        ...
        scale_factor = min(scene_rect.width() / pixmap_rect.width(), scene_rect.height() / pixmap_rect.height())
...
class GraphWindow(QWidget):
    def __init__(self, title, pixmap, stacked_widget, frequencies, show_step_by_step_graph):
        super().__init__()
        self.setWindowTitle(title)
        self.setWindowState(Qt.WindowMaximized)
        ...
        step_button.setStyleSheet("font-size: 18px; padding: 10px; background-color: #f0f0f0;")
...
class StepByStepGraphWindow(QWidget):
    def __init__(self, title, stacked_widget, steps, output_dir):
        ...
        prev_button.setStyleSheet("font-size: 18px; padding: 10px; background-color: #f0f0f0;")
        prev_button.clicked.connect(self.prev_step)
        button_layout.addWidget(prev_button)

        next_button = QPushButton("Next Step")
        next_button.setStyleSheet("font-size: 18px; padding: 10px; background-color: #f0f0f0;")
...
def main():
    ...
    image_path = filedialog.askopenfilename(initialdir=initial_dir, title="Select an image",
    ...
    encoded_r, huffman_tree_r, frequencies_r, steps_r, encoded_data_r = huffman_encode(list(r_channel.getdata()))
    encoded_g, huffman_tree_g, frequencies_g, steps_g, encoded_data_g = huffman_encode(list(g_channel.getdata()))
    encoded_b, huffman_tree_b, frequencies_b, steps_b, encoded_data_b = huffman_encode(list(b_channel.getdata()))
    ...
    save_huffman_tree_graph(huffman_tree_r, frequencies_r, os.path.join(rgb_graphs_path, "Red_Channel_Huffman_Tree"), code_map_r)
    save_huffman_tree_graph(huffman_tree_g, frequencies_g, os.path.join(rgb_graphs_path, "Green_Channel_Huffman_Tree"), code_map_g)
    save_huffman_tree_graph(huffman_tree_b, frequencies_b, os.path.join(rgb_graphs_path, "Blue_Channel_Huffman_Tree"), code_map_b)

    # Save each channel image
    print("Saving each channel image...")
    r_image = Image.merge('RGB', (r_channel, Image.new('L', r_channel.size), Image.new('L', r_channel.size)))
    g_image = Image.merge('RGB', (Image.new('L', g_channel.size), g_channel, Image.new('L', g_channel.size)))
    b_image = Image.merge('RGB', (Image.new('L', b_channel.size), Image.new('L', b_channel.size), b_channel))
    ...
    encoded_text_file_name_r = os.path.join(rgb_codes_path, f"Codigo_Red_{os.path.splitext(os.path.basename(image_path))[0]}.txt")
    with open(encoded_text_file_name_r, 'w') as f:
        f.write(separated_code_r)
    encoded_text_file_name_g = os.path.join(rgb_codes_path, f"Codigo_Green_{os.path.splitext(os.path.basename(image_path))[0]}.txt")
    with open(encoded_text_file_name_g, 'w') as f:
        f.write(separated_code_g)
    encoded_text_file_name_b = os.path.join(rgb_codes_path, f"Codigo_Blue_{os.path.splitext(os.path.basename(image_path))[0]}.txt")
    ...
    entropy_r, avg_length_r, efficiency_r = calculate_efficiency(frequencies_r, code_map_r)
    entropy_g, avg_length_g, efficiency_g = calculate_efficiency(frequencies_g, code_map_g)
    entropy_b, avg_length_b, efficiency_b = calculate_efficiency(frequencies_b, code_map_b)
    print(f"Red channel - Entropy: {entropy_r:.4f}, Average length: {avg_length_r:.4f}, Efficiency: {efficiency_r:.4f}")
    print(f"Green channel - Entropy: {entropy_g:.4f}, Average length: {avg_length_g:.4f}, Efficiency: {efficiency_g:.4f}")
    print(f"Blue channel - Entropy: {entropy_b:.4f}, Average length: {avg_length_b:.4f}, Efficiency: {efficiency_b:.4f}")
    ...
    huffman_coding_file_path_r = os.path.join(rgb_graphs_path, "Huffmans_Coding_Red.txt")
    ...
    huffman_coding_file_path_g = os.path.join(rgb_graphs_path, "Huffmans_Coding_Green.txt")
    ...
    huffman_coding_file_path_b = os.path.join(rgb_graphs_path, "Huffmans_Coding_Blue.txt")
...
def create_gui(subfolder_path, original_image_path, frequencies_r, frequencies_g, frequencies_b):
    app = QApplication(sys.argv)

    # Load images
    original_image = Image.open(original_image_path)
    red_image = Image.open(os.path.join(subfolder_path, "splitted_images", "Red_Channel.jpg"))
    green_image = Image.open(os.path.join(subfolder_path, "splitted_images", "Green_Channel.jpg"))
    blue_image = Image.open(os.path.join(subfolder_path, "splitted_images", "Blue_Channel.jpg"))
    restored_image = Image.open(os.path.join(subfolder_path, "restored_image.jpg"))

    # Load Huffman tree graphs
    red_graph = Image.open(os.path.join(subfolder_path, "rgb_graphs", "Red_Channel_Huffman_Tree.png"))
    green_graph = Image.open(os.path.join(subfolder_path, "rgb_graphs", "Green_Channel_Huffman_Tree.png"))
    blue_graph = Image.open(os.path.join(subfolder_path, "rgb_graphs", "Blue_Channel_Huffman_Tree.png"))
    ...
    button_layout.addWidget(QPushButton("Original Image", clicked=lambda: show_window("Original Image", original_pixmap, "Original Image")), 0, 0)
    button_layout.addWidget(QPushButton("Red Channel", clicked=lambda: show_window("Red Channel", red_pixmap, read_file("Huffmans_Coding_Red.txt", subfolder_path))), 0, 1)
    button_layout.addWidget(QPushButton("Green Channel", clicked=lambda: show_window("Green Channel", green_pixmap, read_file("Huffmans_Coding_Green.txt", subfolder_path))), 1, 0)
    button_layout.addWidget(QPushButton("Blue Channel", clicked=lambda: show_window("Blue Channel", blue_pixmap, read_file("Huffmans_Coding_Blue.txt", subfolder_path))), 1, 1)
    button_layout.addWidget(QPushButton("Restored Image", clicked=lambda: show_window("Restored Image", restored_pixmap, "Restored Image")), 2, 0)
    button_layout.addWidget(QPushButton("Red Channel Graph", clicked=lambda: show_graph("Red Channel Huffman Tree", red_graph_pixmap, frequencies_r)), 2, 1)
    button_layout.addWidget(QPushButton("Green Channel Graph", clicked=lambda: show_graph("Green Channel Huffman Tree", green_graph_pixmap, frequencies_g)), 3, 0)
    button_layout.addWidget(QPushButton("Blue Channel Graph", clicked=lambda: show_graph("Blue Channel Huffman Tree", blue_graph_pixmap, frequencies_b)), 3, 1)
    button_layout.addWidget(QPushButton("Tuple Encoding", clicked=lambda: show_tuple_encoding(subfolder_path, original_image_path, stacked_widget, show_step_by_step_graph)), 4, 0, 1, 2)

    # Add some styling to the buttons
    for i in range(button_layout.count()):
        button = button_layout.itemAt(i).widget()
        button.setStyleSheet("font-size: 20px; padding: 15px; background-color: #f0f0f0;")
    ...
    def show_graph(title, pixmap, frequencies):
        window = GraphWindow(title, pixmap, stacked_widget, frequencies, show_step_by_step_graph)
        windows.append(window)
        stacked_widget.addWidget(window)
        stacked_widget.setCurrentWidget(window)
        window.showMaximized()
    ...
    def show_step_by_step_graph(title, frequencies):
        output_dir = os.path.join(subfolder_path, "step_by_step_graphs", title.replace(" ", "_"))
...
def generate_steps_for_graph(frequencies, output_dir):
    steps = []
    nodes = [HuffmanNode(symbol, weight, word=symbol) for symbol, weight in frequencies.items()]
    heapq.heapify(nodes)
    ...
    for node in sorted(nodes, key=lambda x: x.weight):
        add_node_to_graph(initial_dot, node, added_edges=added_edges, total_weight=total_weight)
    try:
    ...
    while len(nodes) > 1:
        ...
        added_edges = set()
        for node in sorted(nodes, key=lambda x: x.weight):
            add_node_to_graph(dot, node, added_edges=added_edges, total_weight=total_weight)
        add_node_to_graph(dot, new_node, added_edges=added_edges, total_weight=total_weight)
        try:
            dot.render(filename=os.path.join(output_dir, f"step_{step_count}"), format='png')
        except graphviz.backend.ExecutableNotFound:
            print(f"Warning: Graphviz executable not found. Skipping step {step_count}.")
        except Exception as e:
            print(f"Warning: An error occurred while rendering the graph for step {step_count}: {e}")
        step_count += 1
    ...
    added_edges = set()
    add_node_to_graph(final_dot, root, added_edges=added_edges, code_map=code_map, total_weight=total_weight, final_iteration=True)
    try:
        final_dot.render(filename=os.path.join(output_dir, f"step_{step_count}"), format='png')
    except graphviz.backend.ExecutableNotFound:
...
def add_node_to_graph(dot, node, parent=None, edge_label=None, added_edges=None, code_map=None, total_weight=None, final_iteration=False):
    if node is not None:
        if node.word is not None:  # Leaf node
            code = code_map[node.word] if final_iteration and code_map and node.word in code_map else ""
        ...
        add_node_to_graph(dot, node.left, parent=node, edge_label='0', added_edges=added_edges, code_map=code_map, total_weight=total_weight, final_iteration=final_iteration)
        add_node_to_graph(dot, node.right, parent=node, edge_label='1', added_edges=added_edges, code_map=code_map, total_weight=total_weight, final_iteration=final_iteration)
...
def show_tuple_encoding(subfolder_path, image_path, stacked_widget, show_step_by_step_graph):
    ...
    timestamp = datetime.now().strftime("%d%m%Y_%H%M%S")
    subfolder_name = f"Tuple_{timestamp}_{os.path.splitext(os.path.basename(image_path))[0]}"
    subfolder_path = os.path.join(subfolder_path, subfolder_name)
    os.makedirs(subfolder_path, exist_ok=True)

    # Save the encoded image text as a .txt file
    print("Creating text file for encoded text...")
    encoded_text_file_name = os.path.join(subfolder_path, f"Tuple_Codigo_{os.path.splitext(os.path.basename(image_path))[0]}.txt")
    with open(encoded_text_file_name, 'w') as f:
        f.write(encoded_image)

    # Save the Huffman tree graph
    print("Saving Huffman tree graph...")
    save_huffman_tree_graph(huffman_tree, frequencies, os.path.join(subfolder_path, "Tuple_Huffman_Tree"), code_map)
    ...
    window = GraphWindow("Tuple Encoding Results", graph_pixmap, stacked_widget, frequencies, show_step_by_step_graph)
